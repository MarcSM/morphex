...
void Synthesis::getWindow()
{
    ...
    juce::dsp::WindowingFunction<float>::fillWindowingTables (ow.data(), ow.size(), juce::dsp::WindowingFunction<float>::triangular, false);

    std::vector<float> bh (NS);
    juce::dsp::WindowingFunction<float>::fillWindowingTables (bh.data(), bh.size(), juce::dsp::WindowingFunction<float>::blackmanHarris, false);
    ...
    juce::dsp::WindowingFunction<float>::fillWindowingTables (window.stoc.data(), window.stoc.size(), juce::dsp::WindowingFunction<float>::blackmanHarris, true);
}
...
std::vector<float> Synthesis::generateSines (std::vector<float> iploc, std::vector<float> ipmag, std::vector<float> ipphase, int NS, int fs)
{
    ...
    // Perform the IFFT
    juce::dsp::Complex<float>* Y_harmonics = new juce::dsp::Complex<float>[NS];
...
std::vector<float> Synthesis::generateStocs (std::vector<float> stocs_morph, int H, int NS)
{
    ...
    // Perform the IFFT
    juce::dsp::Complex<float>* Y_stocs = new juce::dsp::Complex<float>[NS];
...
float* Synthesis::generateSine (float frequency, float level, float samples_to_render, int sample_rate, float currentAngle)
{
    ...
    float angle_delta       = cycles_per_sample * 2.0 * juce::MathConstants<float>::pi;
