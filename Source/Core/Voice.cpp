...
namespace morphex
{
Voice::Voice (juce::AudioProcessorValueTreeState& parameters, Instrument& instrument) :
...
bool Voice::canPlaySound (juce::SynthesiserSound* synthSound)
{
...
void Voice::setCurrentPlaybackSampleRate (const double newRate)
{
    //        adsrState.setSampleRate(sampleRate);
}
...
void Voice::clearAndResetCurrentNote()
{
    clearCurrentNote();
    m_synthesis.reset();
    reset();
...
void Voice::updateMidiNoteWithPitchWheel (int newValue, bool set_current)
{
    ...
    if (allow_pitch_wheel)
    {
        float f_new_midi_pitch_wheel = juce::jmap ((float) newValue, 0.0f, 16383.0f, -f_pitch_wheel_range_semitones, f_pitch_wheel_range_semitones);
...
// TODO
void Voice::updateAdsrParams (float attack, float decay, float sustain, float release)
{
    ...
    //        adsrState.setParameters(adsrParams);
}

std::vector<float> Voice::getNextFrame (float f_note, float f_velocity, int i_frame_length, float f_interpolation_factor)
{
    ...
    while (m_synthesis.live_values.i_samples_ready < i_frame_length)
    {
        ...
        //            {
        //                if (adsrState != AdsrState::Sustain and
