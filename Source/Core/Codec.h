...
template <typename T>
void inline decodeVector (std::vector<T>& given_list, int abs_values = false, int decimal_places = Constants::XML_DECIMAL_PLACES)
{
    for (int i = 0; i < given_list.size(); i++)
    {
        // Recover the "-" on mags and stocs
        if (abs_values)
            given_list[i] = -given_list[i];
...
// Matrix Decoding
void inline decodeMatrix (std::vector<std::vector<float>>& given_matrix, bool abs_values = false, bool diff_decoding = true, int decimal_places = Constants::XML_DECIMAL_PLACES)
{
    if (diff_decoding)
        decodeMatrixDiff (given_matrix);
